...
- **spirit-po** is known to work with gcc >= 4.9, clang >= 3.5. It is tested with multiple versions of these compilers.
- **spirit_po** does not require C++ exceptions to be enabled. The tests run when compiled with `-fno-exceptions` flag, provided that
